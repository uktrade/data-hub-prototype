...
class CompanyApp extends React.Component {
  static loadProps (context, cb) {
    ...
    companyService.getCompanyForSource(params.token, params.sourceId, params.source)
      .then((company) => {
    ...
    return (
      <div>
        ...
        { ((!company.archived && path === 'contacts' && !company.id) ||
           (!company.archived && path === 'interactions' && !company.id)) && this.companyRequiredForSection()
        }
