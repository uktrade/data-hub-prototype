...
function accountManagerLookup (req, res) {
  ...
  advisorRepository.advisorSearch(req.session.token, term)
    .then((results) => {
      // django only returns results for cotains, so filter out
      // the ones that dont start with
      res.json(results)
    })
}
...
function contactLookup (req, res) {
  ...
  companyService.getDitCompany(req.session.token, companyParam)
    .then((company) => {
