...
export default class AddressControl {
  ...
  constructor(element, postcodeUrl='/postcodelookup/') {
    ...
    this.addEvents();
    new Autocomplete(this.addressCountryInput, countrys);
  ...
  setInitialState () {
    ...
    this.address1Input = this.element.find(`[name="${this.name}_address_1"]`);
    this.address2Input = this.element.find(`[name="${this.name}_address_2"]`);
    this.addressTownInput = this.element.find(`[name="${this.name}_address_town"]`);
    this.addressCountyInput = this.element.find(`[name="${this.name}_address_county"]`);
    this.addressPostcodeInput = this.element.find(`[name="${this.name}_address_postcode"]`);
    this.addressCountryInput = this.element.find(`[name="${this.name}_address_country"]`);
    this.errors = {};
  ...
  showAddress () {
    ...
    this.addressTownInput.parent().show();
  ...
  hideAddress () {
    ...
    this.addressTownInput.parent().hide();
  ...
  updateVisibility () {
    const countryId = this.addressCountryInput.val();
    let country;
    if (countryId.length > 0) {
      country = this.lookupCountry(countryId);
    }

    if (this.address1Input.val().length > 0 ||
        this.address2Input.val().length > 0 ||
        this.addressTownInput.val().length > 0 ||
        this.addressCountyInput.val().length > 0 ||
        this.addressPostcodeInput.val().length > 0 ||
        this.forceManualEntry ||
        country && country.name.toLocaleLowerCase() !== 'united kingdom')
    {
      this.showAddress();
    } else {
      this.hideAddress();
    ...
    if (country && country.name.toLocaleLowerCase() === 'united kingdom') {
  ...
  postcodeLookupResultHandler(result) {
    ...
    this.addressTownInput.val(result[0].city);
  ...
  validate() {
    ...
    if ( !this.optionalNonUk && country !== 'united kingdom' || country === 'united kingdom') {
      ...
      if (!this.addressTownInput.val() || this.addressTownInput.val().length === 0) {
        this.errors.town = {
          message: "Address incomplete",
          field: this.addressTownInput
        };
      }
    }
