...
class Autocomplete {
  ...
  select() {
    ...
    if (suggestion.length === 0) {
      suggestion = this.$menu.find('.autosuggest__suggestion:first-child');
    }
    const val = suggestion.attr('data-value');
    const display = suggestion.text();
  ...
  lookupLocalArray(term) {
    ...
    if (this.options[0].id) {
      matchingItems = this.options.filter((item) => {
  ...
  render(items) {
    ...
    let suggestionElements = items.slice(0, this.maxItems).map((item) => {
      ...
      return $(`<li class="autosuggest__suggestion" data-value="${value}"><a href="#">${highlighter(text)}</a>`);
